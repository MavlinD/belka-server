...
async def test_cant_create_ad(
    ...
    resp = await client.put(
        routes.create_ad,
        json={"name": name_ad * 30, "price": 123, "desc": f"desc of ind {name_ad}"},
...
async def test_cant_create_ad_with_err_price(
    ...
    resp = await client.put(
        routes.create_ad,
        json={"name": name_ad, "price": "fake price", "desc": f"desc of ind {name_ad}"},
        headers=user_active_auth_headers,
    )
    log.debug(resp)
    data = resp.json()
...
async def test_cant_create_ad_with_err_price_2(
    ...
    resp = await client.put(
        routes.create_ad,
        json={"name": name_ad, "price": 123.876687908, "desc": f"desc of ind {name_ad}"},
        headers=user_active_auth_headers,
    )
    log.debug(resp)
    data = resp.json()
